                                                  ...
                                                  BlocProvider(
                                                      create: (context) =>
                                                          AuctionPageSliderController(
                                                              10000)),
                                                  BlocProvider(
                                            create: (context)=>AuctoinImgController())
                                                ],
                                                child: AuctionPage(
                                                  auctionid: state[index].auctionid,
                                                  title: state[index].title,
                                                  content: state[index].content,
                                                  seller: state[index].seller_id,
                                                  priceNow: state[index].pricenow,
                                                  priceStart:
                                                      state[index].pricestart,
                                                  insertDate:
                                                      state[index].insertdate,
                                                  issuccessed:
                                                      state[index].issuccessed,
                                                  pic: state[index].pic,
                                                ),
                                              );
                                            }), //end of builder,
    ...
    Navigator.of(ctx).push(
      MaterialPageRoute(
        builder: (context) {
          return MultiBlocProvider(
            providers: [
              BlocProvider(create: (context) => OpenAuctionImgController(null),),
